...
(function () {
  ...
  function normalize() {
    // core set for the chosen game is always in play; drop other-game sets
    const core = coreOf(state.game);
    for (const e of DI.expansions) {
      if (e.game !== state.game) state.exps.delete(e.id);
    }
    state.exps.add(core);
    ...
    for (const mod of DI.modules) {
      ...
      if (mod.game !== state.game || !state.exps.has(mod.requires)) state.mods.delete(mod.id);
    }
  }
  ...
  function renderModules() {
    ...
    let shown = 0;
    for (const mod of DI.modules) {
      if (mod.game !== state.game || !state.exps.has(mod.requires)) continue;
      shown++;
  ...
  function docVisible(x, c) {
    switch (x) {
      ...
      case "ix": return c.game === "imperium" && c.has("ix");
      case "imm": return c.game === "imperium" && c.has("imm");
